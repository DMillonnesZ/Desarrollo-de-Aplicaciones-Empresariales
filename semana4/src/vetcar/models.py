...
validar_dni = RegexValidator(r'^\d{8}$', 'El DNI debe tener exactamente 8 dígitos numéricos.')
validar_telefono = RegexValidator(r'^9\d{8}$', 'El teléfono debe tener 9 dígitos y empezar con 9.')
validar_colegiatura = RegexValidator(r'^[A-Z]{2,4}-\d{4,6}$', 'Formato de colegiatura inválido, ej. "ABC-12345".')
validar_solo_letras = RegexValidator(r'^[A-Za-zÁÉÍÓÚáéíóúÑñ ]+$', 'Este campo solo admite letras y espacios.')
...
class Persona(models.Model):
    ...
    telefono = models.CharField(max_length=9, blank=True, validators=[validar_telefono])
...
class Veterinario(Persona):
    colegiatura = models.CharField(max_length=20, unique=True, validators=[validar_colegiatura])
    especialidades = models.ManyToManyField(Especialidad, related_name='veterinarios')
    fecha_ingreso = models.DateField(validators=[validar_fecha_no_futura])  # sin default
...
class Raza(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['nombre', 'especie'], name='raza_unica_por_especie')
        ]
...
class Mascota(models.Model):
    ...
    uuid = models.UUIDField(default=uuid_lib.uuid4, editable=False, unique=True)
    nombre = models.CharField(max_length=80)
    fecha_nacimiento = models.DateField(null=True, blank=True, validators=[validar_fecha_no_futura])
...
class Medicamento(models.Model):
    ...
    precio_unitario = models.DecimalField(max_digits=8, decimal_places=2, validators=[MinValueValidator(0)])
...
class Atencion(models.Model):
    ...
    uuid = models.UUIDField(default=uuid_lib.uuid4, editable=False, unique=True)
    mascota = models.ForeignKey(Mascota, on_delete=models.PROTECT, related_name='atenciones')
...
class Consulta(Atencion):
    ...
    costo = models.DecimalField(max_digits=8, decimal_places=2, validators=[MinValueValidator(0)])  # sin default
    ...
    def clean(self):
        if self.estado == 'ATENDIDA' and not self.diagnostico:
            raise ValidationError('Una consulta atendida debe tener diagnóstico.')
        if self.estado == 'ATENDIDA' and self.fecha > timezone.now():
            raise ValidationError('No se puede marcar como atendida una consulta con fecha futura.')
...
class Vacunacion(Atencion):
    ...
    def clean(self):
        if self.proxima_dosis and self.fecha and self.proxima_dosis <= self.fecha.date():
            raise ValidationError('La próxima dosis debe ser posterior a la fecha de aplicación.')
...
class DetalleReceta(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['consulta', 'medicamento'], name='medicamento_unico_por_consulta')
        ]
...
class SeguimientoClinico(models.Model):
    ...
    fecha_asignacion = models.DateField(validators=[validar_fecha_no_futura])  # sin default
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['mascota', 'veterinario', 'rol'], name='seguimiento_unico')
        ]
...
def auditar_alta_consulta(sender, instance, created, **kwargs):
    if created:
        _registrar_auditoria_atencion(instance, accion='CREACION', estado_nuevo=instance.estado)
